...
public class FibonacciStackCounter
{
     ...
     */
    public static long fib(int n, String prefix, boolean isTopLevel, boolean isLastChild) throws IOException
    {
        ...
        if (isTopLevel) { recursiveTree.append(currentLetter).append(": fib(").append(n).append(")\n"); }
        else
        {
            String visualPrefix = expandPrefix(prefix);
            recursiveTree.append(visualPrefix).append(isLastChild ? "└── " : "├── ").append(currentLetter)
                         .append(": fib(").append(n).append(")\n");
        }
        ...
        String childPrefix = prefix + (isTopLevel ? "" : (isLastChild ? "    " : "│   "));
        char leftLetter = letters[counter];
        long left = fib(n - 1, childPrefix, false, false);
        char rightLetter = letters[counter];
        long right = fib(n - 2, childPrefix, false, true);
        ...
        callStack.removeLast();
        logCallStack();
        if (isTopLevel) { equation = "fib(" + currentLetter + ") = fib(" + leftLetter + ") + fib(" + rightLetter + ")"; }
        return result;
    }
    ...
    private static void openWebpage()
    {
        ...
        catch (IOException e) { System.err.println("Fel när html-filen skulle öppnas: " + e.getMessage()); }
    }
    ...
    private static void appendFibonacciToHtml(int n)
    {
        ...
        if (n > 1) { html.append("Fibonacci(").append(n).append(") = Fibonacci(").append(n - 1).append(") + Fibonacci(").append(n - 2).append(")"); }
        else if (n == 1) { html.append("Fibonacci(1) = 1"); }
        else if (n == 0) { html.append("Fibonacci(0) = 0"); }

        if (!equation.isEmpty()) { html.append("<p>").append(equation).append("</p>"); }
        html.append("</section>");
    }
    ...
    private static String expandPrefix(String prefix)
    {
